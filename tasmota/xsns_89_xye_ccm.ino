/*
  xsns_89_xye_ccm.ino - XYE CCM support for Tasmota
  Copyright (C) 2020  XYE
...
struct XYE
{
  ...
  int cmd_status = 0;
  uint32_t state_150msecond = 0;         // State 130msecond timer
  uint8_t query_id = 0;
  uint8_t set_id = 0;
...
#ifdef XYE_LOG_MORE
    AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("XYE: unexpected packet: 0x%02x, 0x%02x, 0x%02x"),
	      b[2], b[3], b[5]);
#endif
    return;
  }
...
#ifdef XYE_LOG_MORE
    AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("XYE: unexpected reply, got 0x%02x, expected 0x%02x"),
	      b[1], XYE.last_cmd);
#endif
    return;
  }
...
#ifdef XYE_LOG_MORE
    AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("XYE: reply from unexpected device, got 0x%02x, expected 0x%02x"),
	      b[1], XYE.last_dev);
#endif
    return;
  }
...
void CmndXYESet(void)
{
  ...
  val = root[PSTR("operMode")];
  if (val) {
    mask[0x06] = 0xff;
    switch (GetCommandCode(s, sizeof(s), val.getStr(), kXYEoperMode)) {
    case C_OP_OFF:
      cmnd[0x06] = 0x00;
      break;
    case C_OP_AUTO:
  ...
  if (val) {
    mask[0x07] = 0xff;
    switch (GetCommandCode(s, sizeof(s), val.getStr(), kXYEfanMode)) {
  ...
  val = root[PSTR("setTemp")];
  if (val && val.isUint() && mask[0x08] != 0xff) {
    mask[0x08] = 0xff;
    cmnd[0x08] = val.getUInt();
  ...
  if (val) {
    mask[0x09] = 0x03;
    switch (GetCommandCode(s, sizeof(s), val.getStr(), kXYEecoMode)) {
  ...
  val = root[PSTR("timer")];
  JsonParserObject timer_obj = val.getObject();

  val = timer_obj[PSTR("start")];
  if (val) {
    mask[0x0a] = 0xff;
    cmnd[0x0a] = val.getUInt();
  }
  val = timer_obj[PSTR("stop")];
  if (val) {
    mask[0x0b] = 0xff;
    cmnd[0x0b] = val.getUInt();
  }
...
{
  XYESerialInput();
  if (TimeReached(XYE.state_150msecond)) {
    SetNextTimeInterval(XYE.state_150msecond, 150);
    XYEProcess();
  }
}

#ifdef ESP32
TaskHandle_t XYETask;

void XYETaskLoop( void * pvParameters )
{
  for(;;) {
    XYELoop();
    delay(10);
...
void XYETaskSetup()
{
  ...
  xTaskCreatePinnedToCore(
    XYETaskLoop,   /* Task function. */
    "XYETask",     /* name of task. */
    10000,         /* Stack size of task */
    NULL,          /* parameter of the task */
    1,             /* priority of the task */
    &XYETask,      /* Task handle to keep track of created task */
